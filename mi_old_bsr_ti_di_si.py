...
def shuffle_rotate( x):
    ...
    x_h_perm = tf.concat([tf.concat([images_rotate(strip[i], tf.truncated_normal([tf.shape(x)[0]], stddev=FLAGS.max_angles), interpolation='BILINEAR') for i in height_perm],axis=2 ) for strip in x_spilt_h_l],axis=1)
    return x_h_perm
def admix(x):
    indices = tf.range(start=0, limit=tf.shape(x)[0], dtype=tf.int32)
    return tf.concat([(x + FLAGS.portion * tf.gather(x, tf.random.shuffle(indices))) for _ in range(FLAGS.size)], axis=0)

def scale_invariant(x):
    """
    Apply scale-invariant transformation to the input tensor.
    """
    scales = [0.9, 1.0, 1.1]
    scaled_images = []
    
    for scale in scales:
        new_size = tf.cast(tf.shape(x)[1:3], tf.float32) * scale
        new_size = tf.cast(new_size, tf.int32)
        scaled_image = tf.image.resize(x, new_size)
        scaled_image = tf.image.resize_with_crop_or_pad(scaled_image, tf.shape(x)[1], tf.shape(x)[2])
        scaled_images.append(scaled_image)
    
    return tf.reduce_mean(tf.stack(scaled_images), axis=0)
